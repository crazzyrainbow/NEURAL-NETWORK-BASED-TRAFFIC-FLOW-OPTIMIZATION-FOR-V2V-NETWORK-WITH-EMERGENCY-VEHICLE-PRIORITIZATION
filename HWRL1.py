...
def run_rl_simulation(sumo_cfg, agent, steps=1000):
    try:
        ...
        for step in range(steps):
            ...
            reward = -state[0]  # Reward is negative congestion (more vehicles -> worse reward)
...
# Function to manage traffic lights based on action and prioritize ambulances
def set_traffic_light(action, traffic_data):
    tls_ids = ["J1", "J2"]  # Traffic light IDs (adjust as per your setup)
    states = [
    ...
    for tls_id in tls_ids:
        if traffic_data[:, 1].sum() > 0:  # If there are any ambulances
            ...
            # Manage lane switching for non-emergency vehicles
            for lane_id in traci.trafficlight.getControlledLanes(tls_id):
                for veh_id in traci.lane.getLastStepVehicleIDs(lane_id):
                    ...
                    elif traci.vehicle.getTypeID(veh_id) != 'ambulance':  # For non-ambulance vehicles
                        try:
                            traci.vehicle.changeLane(veh_id, 1, 25.0)  # Change lane to allow ambulance
                            print(f"Vehicle {veh_id} changed lane to allow ambulance to pass.")
                        except traci.exceptions.TraCIException:
                            print(f"Vehicle {veh_id} could not change lane.")
